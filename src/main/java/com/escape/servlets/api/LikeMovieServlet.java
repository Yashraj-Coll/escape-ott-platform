    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            if (session == null || session.getAttribute("userId") == null) {
                ...
                responseData.put("error", "Please sign in/sign up to like movies");
            ...
            if (!"admin".equals(userRole)) {
                ...
                if (subscriptionPlan == null || subscriptionPlan.equals("Free")) {
                    responseData.put("success", false);
                    responseData.put("error", "Please upgrade to a premium plan to like movies");
                    responseData.put("shouldRedirect", true);
            ...
            // Toggle like status
            boolean success = MovieService.toggleLike(userId, movieId);
            
            if (success) {
                responseData.put("success", true);
                responseData.put("message", "Movie like status updated successfully");
                // Get updated like count
                Map<String, Object> movieStats = MovieService.getMovieStats(movieId);
                responseData.put("likeCount", movieStats.get("likes"));
            } else {
                responseData.put("success", false);
                responseData.put("error", "Failed to update like status");
            }

        } catch (Exception e) {
            ErrorHandler.logError("Error in LikeMovieServlet", e);
            responseData.put("success", false);
